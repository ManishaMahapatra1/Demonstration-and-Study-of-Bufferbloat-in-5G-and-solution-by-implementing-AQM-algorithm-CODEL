...
Time cur = Time(0);  // used to monitor the time of the first packet experiencing a queue delay greater than threshold when in a non dropping phase
int count_new = 0;   //used to maintain count of the dropped packets in a dropping phase
int next_drop_time =0;   //used to store the next time at which a packet should be dropped
Time cur_qdelay =Time(0); // used to maintain the current queue delay
Time interval = Time(100000000);  // used to maintain the interval
Time threshold = Time(100000000); // used to maintain the threshold
...
LteRlcUm::DoTransmitPdcpPdu (Ptr<Packet> p)
{
  ...
  //Here we check to see if the queue delay is greater than the threshold
  if (cur_qdelay > threshold)
      {
                
                if (cur.GetMilliSeconds() == 0)
                {
                        // updating the first spotted instance having queue delay greater than threshold
                        cur = Simulator::Now();
                }
                else if( Simulator::Now() - cur > interval )
                {
                        // simulating the dropping phase
                        if(count_new == 0)
                        {
                                //updating the drop count and drop time everytime we drop a packet
                                count_new+=1;
                                next_drop_time = Simulator::Now().GetMilliSeconds() +interval.GetMilliSeconds()/sqrt(count_new);
                                return;
                        }
      ...
      {
                // resetting the values in case of receiving a packet with delay less than threshold
                count_new = 0;
                next_drop_time=0;
                cur = Time(0);
      }
...
LteRlcUm::DoNotifyTxOpportunity (LteMacSapUser::TxOpportunityParameters txOpParams)
{
  ...
  // Here we update the current queue delay based on the packet which is about  to be transmitted 
  cur_qdelay = Simulator::Now() - firstSegmentTime;
  //std::cout<<cur_qdelay.GetNanoSeconds() <<std::endl;
