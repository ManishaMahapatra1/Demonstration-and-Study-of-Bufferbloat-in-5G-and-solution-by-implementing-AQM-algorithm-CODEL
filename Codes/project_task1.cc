...
// ./ns3 run "project_task1.cc"&>123.txt
using namespace ns3;
#include "ns3/log.h"
...
{
  // enable logging or not// enable logging
  bool logging = true;
  if (logging)
    {
  ...
  double txPower = 43; //43
  double lambda = 2500;
  uint32_t udpPacketSize = 1500; //1500
  ...
  if (singleUeTopology)
    {
      ...
      utPositionAlloc->Add (Vector (0.0, 10.0, ueHeight));
    }
  else
    {
  ...
  // NetDeviceContainer internetDevices = p2ph.Install (pgw, router);
  // 
  // NetDeviceContainer internetDevices2 = p2ph.Install (router, remoteHost);
  ...
  UdpClientHelper dlClient2;
  dlClient2.SetAttribute ("RemotePort", UintegerValue (dlPort));
  dlClient2.SetAttribute ("PacketSize", UintegerValue (udpPacketSize));
  dlClient2.SetAttribute ("MaxPackets", UintegerValue (0xFFFFFFFF));
  
  //Ensuring that two dlclients are prensent each have having their respective parameter of the required flow

  lambda1 = 1.0              // mice flow
  lambda2 = 30000.0          // elephant flow
  ...
  for (uint32_t i = 0; i < ueNodes.GetN (); ++i)
    {
      ...
      // Adding the same destination for both the dlclients ( UE)
      dlClient.SetAttribute ("RemoteAddress", AddressValue (ueAddress));
      dlClient2.SetAttribute ("RemoteAddress", AddressValue (ueAddress));
  ...
  // This is the commented code line that is used to generate the pcap for the UE
